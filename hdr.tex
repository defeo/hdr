...
get to the primitive known as SIDH, the building block of SIKE, one of
the candidates to the NIST call for post-quantum public key
encryption. %
...
understand security. %
An adversary who only sees the public keys, can break the system by
solving the following problem.%
\footnote{Security proofs for the Couveignes--Rostovtsev--Stolbunov
  protocol rely on variants of the isogeny walk problem similar to DDH
  or CDH.}

\begin{problem}[Isogeny walk problem]
  Given two elliptic curves $E,E'∈\Ell_q(\O)$, find an isogeny path
  $E→E'$.
\end{problem}

If we want to avoid brute force attacks, we need $\Ell_q(\O)$ to be
large, hence $q$ must also be large. %
When $E,E'$ are uniformly random in $\Ell_q(\O)$, the best attacks to
the isogeny walk problem take $O(q^{1/4})$
operations~\cite{GHS,galbraith+stolbunov11}. %
However, we must also ensure that the key space covers the whole
$\Ell_q(\O)$, possibly approaching the uniform distribution. %
This means that we need a relatively large subset $S⊂\Cl(\O)$: for
example, if walks are sampled by picking a random subset of $S$ and
composing the associated isogenies, the key space will contain at most
$2^{\#S}$ distinct keys. %
...
% LocalWords:  cryptographic codomain invertible diagonalizes
% LocalWords:  expander expanders polynomially invariants
