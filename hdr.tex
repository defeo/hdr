...
contrary to what the figure suggests, this graph has no ``origin''.
...
graphs, and makes them $ℓ+1$ regular; but will they look the same as
Figure~\ref{fig:infinite-tree}? %
Interestingly, no. %
...
for $0≤a<ℓ$, describing the neighborhood of $E$. %
If we now look at the matrices isogenies of degree $ℓ^n$, quotienting
out scalar multiplications, we obtain the tree structure\footnote{I am
  grateful to J. Plût for explaining this to me, and providing the
  TikZ code for Figure~\ref{fig:serre-tree}} on
$\PGL_2(ℚ_ℓ)/\PGL_2(ℤ_ℓ)$ shown in Figure~\ref{fig:serre-tree}, a
structure that Serre~\cite[II]{SL2} calls the tree of $\SL_2$ over
$ℤ_ℓ$. %
...
Which path in the tree is horizontal?
\begin{table}
  \centering
  \begin{tabular}{l}
...
% LocalWords:  undirected bathymeter endomorphisms tensored
% LocalWords:  quotienting
