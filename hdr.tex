...
logarithmic in $q$), craters tend to be ``big'' (polynomial in $q$). %
...
$\bar{\F}_q$ of curves with complex multiplication by $\O$, and we
will assume that it is non-empty. %

Let $\a$ be an invertible ideal in $\End(E)≃\O$, and define the
\emph{${\a}$-torsion} subgroup of $E$ as
\begin{equation*}
  \label{eq:a-torsion}
  E[\a] = \{P ∈ E(\bar{\F}_q) \mid σ(P) = 0
  \text{ for all } σ ∈ \a \}.
\end{equation*}
This subgroup is the kernel of an isogeny $\phi_{\a}:E→E/E[\a]$; it
can be proven that $\phi_{\a}$ is horizontal, and that its degree is
the \emph{norm} of $\a$ as an ideal of $\End(E)$. %

We will write $\a·E$ for the isomorphism class of $E/E[\a]$; it is
easy to extend this to an action
$·:\mathcal{I}(\O)×\Ell_q(\O)→\Ell_q(\O)$ of the group of invertible
ideals of $\O$ on $\Ell_q(\O)$. %
...
\section{Key exchange from complex multiplication graphs}
...
\section{Other protocols, crypto}
...
% LocalWords:  quotienting eigenspace coprime submodule Atkin
