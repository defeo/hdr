...
those that are called \emph{Elkies primes} (more on this later). %
...
If $E/\F_p$ is an ordinary curve, $E[p^k]≃ℤ/p^kℤ$ has a particularly
simple structure. %
...
Given that the $p^k$-torsion groups are cyclic, at most $O(p^k)$
...
Lercier' algorithm only works when $p=2$.
...
Nevertheless, Couveignes' algorithm still presents some theoretical
interest. %
As we shall see by the end of this chapter, its algebraic nature,
