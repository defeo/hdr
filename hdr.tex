...
Isomorphism algorithms let us lift the restriction to special
polynomials: using them, a computer algebra system may internally
represent finite fields with special polynomials, while letting the
user work with a isomorphic copy defined by a polynomial of their
choice. %
...
An thorough review of all known algorithms is given in the paper
...
two finite fields $k=\F_p[X]/f(X)$ and $K=\F_p[Y]/g(Y)$ such that
$\deg f$ divides $\deg g$. %
We decompose it in two subproblems.

\begin{problem}[Embedding description]
  Given $k=\F_p[X]/f(X)$ and $K=\F_p[Y]/g(Y)$, determine elements
  $α∈k$ and $β∈K$ such that $k=\F_p(α)$, and such
  that there exists an embedding $ϕ$ mapping $α$ to
  $β$.
\end{problem}
...
\begin{problem}[Embedding evaluation]
  Given an \emph{embedding description} $(\alpha,\beta)∈k×K$, given
  $γ∈k$ and $δ∈K$, solve the following problems:
  \begin{itemize}
  \item Compute $ϕ(γ)∈K$.
  \item Test if $δ∈ϕ(k)$.
...
The algorithms for the embedding description are very similar to those
to compute irreducible polynomials. %
...
% LocalWords:  Kummer Shoup composita monomial
