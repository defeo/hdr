...
$E$ is supersingular. %
$Δ_π$ is called the \emph{discriminant of $π$}, or, by a slight abuse,
...
We thus are in one of the following four possibilities:
\begin{itemize}
...
\item[(2)] $π$ has two distinct eigenvalues, i.e., it is conjugate to
  a diagonal matrix
  $\left(\begin{smallmatrix}λ&0\\0&μ\end{smallmatrix}\right)$ with
  $\lambda\neq\mu$; then $E$ has two $\ell$-isogenies.
\end{itemize}

Naturally, the number of eigenvalues of $π$ depends the factorization
of the polynomial $x^2-tx+q$ over $\F_ℓ$, or equivalently on the
Legendre symbol $\leg{Δ_π}{ℓ}$; the correspondence is summarized in
Table~\ref{todo}. %
The four cases can also be read in the way the modular polynomial
$Φ_ℓ(j(E),y)$ factors over $\F_q$; this is at the heart of Atkin's
improvement to Schoof's point counting
algorithm~\cite{atkin91,atkin92}. %

However, not all isogenies are born equal, and the Frobenius
endomorphism can reveal even more of their properties.

If $E$ is ordinary, $ℚ[π]$ is an imaginary quadratic extension of the
rationals, and it contains $\End(E)$ as an order:
\[ℤ[π] ⊂ \End(E) ⊂ \O_K.\]
The situation is richer for supersingular curves, and we shall come
back to it in Chapter~\ref{cha:crypto}.
...
\begin{table}
  ...
  \end{tabular}
  \caption{Number and types of $ℓ$-isogenies, according to curve
    discriminant.}
...
% LocalWords:  Schoof endomorphism Frobenius supersingular
% LocalWords:  diagonalizing diagonalizable diagonalization
