...
Said otherwise, we derive a projection algorithm by \emph{transposing}
the embedding one. %
...
$k≃K$; we refer to appendix for the general problem. %
...
in~\cite{brieulle2018computing}, we have not been able to find a
single example where elliptic periods do not generate their field of
definition. %
...
to conjecture that this probability may actually be zero, as is
precisely stated in the appendix. %
...
in time $\tildO(n^{(ω+1)/2}+n\log q)$, where $ω$ is the \emph{exponent
  of linear algebra}, while the elliptic variant runs in time
$\tildO(n^2\log q)$. %
...
% LocalWords:  Allombert Allombert's
