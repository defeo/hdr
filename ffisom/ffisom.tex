...
{\small This article was authored with Ludovic Brieulle, Javad
  Doliskani, Jean-Pierre Flori and Éric Schost. It originally appeared
  in \textit{Mathematics of Computation}, 88 (2019),
...
Section~\ref{ffisom:sec:kummer} describes the family of algorithms based
...
interest. %
We present instead in Subsection~\ref{ffisom:sec:fast-algor-large} a simpler
algorithm with essentially the same complexity.

In Section~\ref{ffisom:sec:rains-algorithm} we describe Rains' algorithm. %
...
interest in Appendix~\ref{ffisom:app:rains-vars}.

Then, in Section~\ref{ffisom:sec:rains-elliptic} we present a generalization
...
Conjecture~\ref{ffisom:conj:ellperiods}).

Section~\ref{ffisom:sec:selection} does a global comparison of all the
...
Section~\ref{ffisom:sec:experimental-results}, where we compare our
...
\label{ffisom:sec:preliminaries}

\subsection{Fundamental algorithms and complexity}
\label{ffisom:sec:fundamentalgo}
...
\begin{note}\label{ffisom:note:multimc}
...
\item
  ...
  $sT_Q$, but as pointed out in Note~\ref{ffisom:note:multimc}, improvements 
...
\item
  ...
  Note~\ref{ffisom:note:multimc}, that we denote $T_{Q',m}$.
...
\begin{proof}
  ...
  The trace in Eq.~\eqref{ffisom:equ:trace-simple} is computed as the
...
\begin{algorithm}
  \caption{Kaltofen--Shoup EDF for extension fields}
	...
	\begin{algorithmic}[1]
		...
		\STATE\label{ffisom:alg:ks-pseudotrace} Compute $\displaystyle a_1 
		\leftarrow \sum_{i=0}^{md-1} a_0^{q^i} \mod h$,
		\IF{$q$ is an even power $q=2^e$}
		\STATE\label{ffisom:alg:ks:even} Compute $\displaystyle a_2 \leftarrow 
		\sum_{i=0}^{e-1} a_1^{2^i}\mod h$
		\ELSE
		\STATE\label{ffisom:alg:ks:odd} Compute $a_2 \leftarrow a_1^{(q-1)/2}\mod h$
		\ENDIF
		\STATE\label{ffisom:alg:ks:gcd} Compute $h_0\leftarrow\gcd(a_2,h)$ and 
...
\end{proposition}
\begin{proof}
We use Algorithm~\ref{ffisom:alg:ks} with $k=\F_q(\zeta)$, to get a linear
factor of the polynomial $Z^r-\alpha$, so that $d=1$ (note that
$Z^r-\alpha$ splits into linear factors in $k[Z]$). We discuss
Step~\ref{ffisom:alg:ks-pseudotrace}, which is the dominant step. Let $f \in
...
Steps~\ref{ffisom:alg:ks:odd} and~\ref{ffisom:alg:ks:even} are again an instance of
Case~5, and their cost is subsumed by Step~\ref{ffisom:alg:ks-pseudotrace}.
The depth of the recursion in Algorithm~\ref{ffisom:alg:ks} is $\log(r)$, and
...
\begin{proposition}\label{ffisom:prop:root_high_degree_extension}
...
\ref{ffisom:alg:ks} for the special case $d = 1$. Using our discussion on the
...
is treated in Section~\ref{ffisom:sec:fast-kummer}, the case $r = p^d$, where $d$ 
is a positive integer, is treated in Section~\ref{ffisom:sec:fast-artin-schreier}.
Finally, in Section~\ref{ffisom:sec:fast-algor-large} we give a variant of the case
...
a variant which is similar to our Proposition~\ref{ffisom:prop:xitheta}
...
$\F_q(\zeta)$ is done using Proposition~\ref{ffisom:prop:root-fpz}, so we are
...
\label{ffisom:sec:algor-rely-line}
...
of $\hat{b}$ is done using Eq.~\eqref{ffisom:eq:hprime} and the matrix of
...
Using the polynomial $b(S)$ introduced in Eq.~\eqref{ffisom:eq:allomb-b},
and defining
$g(S)=(S^r-1)/h(S)$, we can rewrite Eq.~\eqref{ffisom:eq:theta90} as
\begin{equation}
  \label{ffisom:eq:theta90bis}
...
and compute $\theta_a = \hat{b}(\tau_a)$ using Eq.~\eqref{ffisom:eq:hprime},
...
functions in Proposition~\ref{ffisom:prop:trace-like}, to directly evaluate
$\theta_a$ using Eq.~\eqref{ffisom:eq:thetaa}. For $j \ge 1$, let
...
recursive relations:
\begin{equation}
\label{ffisom:eq:theta-recursive}
(\xi_{i+j}, \theta_{a, i+j}) = (\sigma^i(\xi_j), \theta_{a,i} + \sigma^i(\theta_{a,j})\zeta^{-i}).
\end{equation}

\begin{proposition}
  \label{ffisom:prop:xitheta}
  Given $a\in k$, the value $\theta_a$ in Eq.~\eqref{ffisom:eq:thetaa} can be
  computed using \[
...
\begin{proof}
  ...
  To compute the recursive formulas in Eq.~\eqref{ffisom:eq:theta-recursive}
  we use the same technique as in Proposition~\ref{ffisom:prop:trace-like}:
  ...
  modular compositions in $k$, at a cost of $O(s^{(\omega-1)/2}r^{(\omega+1)/2})$ operations (see Note~\ref{ffisom:note:multimc}). %
...
We use Eq.~\eqref{ffisom:eq:theta90bis} and Eq.~\eqref{ffisom:eq:hprime}
to compute $\theta_a$ as
$\theta_a = \hat{b}\circ \hat{g} (a)$.
\begin{proposition}
  \label{ffisom:prop:ks-theta}
  Given $a\in k$, the value $\theta_a$ in Eq.~\eqref{ffisom:eq:thetaa} can
  be computed using
  \[
...
\begin{proof}
  ...
  We then use Eq.~\eqref{ffisom:eq:hprime} to compute
...
\begin{proposition}
  \label{ffisom:prop:iter-frob-theta}
  Given $a\in k$, the value $\theta_a$ in Eq.~\eqref{ffisom:eq:thetaa} can
  be computed using 
  \[
...
\begin{proposition}
  \label{ffisom:proposition:XiDelta-updated}
  Given $k,K$ of degree $r$ over $\F_q$, assuming that $s$ is the
  order of $q$ in $(\Z/r\Z)^\times$,   Algorithm~\ref{ffisom:alog:allombert} 
...
\begin{proof}
  ...
  $\F_q(\zeta)$ is given by Proposition~\ref{ffisom:prop:root-fpz} and can
  not be neglected.
  ...
  \item If we use the algorithm described in
    our first solution, combining Proposition~\ref{ffisom:prop:xitheta} with the first
    case of Proposition~\ref{ffisom:prop:root-fpz}, we obtain an estimate of
    $O(s^{(\omega-1)/2}r^{(\omega+1)/2}\log(r)+\MM(r)\log(q))$
    operations.
%% $\MM(sr)\log(s)\log(r)$ is $o(s^{(\omega-1)/2}r^{(\omega+1)/2})$
  \item If we use the algorithm described in
    our second solution, combining Proposition~\ref{ffisom:prop:ks-theta} with the first
    case of Proposition~\ref{ffisom:prop:root-fpz}, we obtain an estimate of
    $O( r^{(\omega^2-4\omega-1)/(\omega-5)}+(s+r^{2/(5-\omega)})\MM(r)\log(q)
+s^{\omega-1}r\log(r)\log(s)+\MM(rs)\log(r)\log(s))$
    operations.
  \item Otherwise, we use the algorithm described in our third solution. Combining
    Proposition~\ref{ffisom:prop:iter-frob-theta} with the second case of
    Proposition~\ref{ffisom:prop:root-fpz}, and replacing $s$ with $r$
...
\label{ffisom:sec:fast-artin-schreier}
...
\begin{equation}
	\label{ffisom:equ:art-sch-tower}
...
\begin{equation}
	\label{ffisom:equ:art-sch-hil}
	\alpha = \frac{1}{\trace(\theta)}[a\sigma(\theta) + (a + \sigma(a))\sigma^2(\theta) + \cdots + 
	(a + \sigma(a) + \cdots + \sigma^{rt - 2}(a))\sigma^{rt - 1}(\theta)]
\end{equation}
where $t = [\F_q : \F_p]$.
To compute $\alpha$ using Eq.~\eqref{ffisom:equ:art-sch-hil} efficiently, for $j \ge 1$, we define
$ \xi_j = \sigma^j(x)$, and, 
for $u$ in $k$, 
...
Now, to compute $\alpha_d$ in Eq.~\eqref{ffisom:equ:art-sch-tower} we 
...
\label{ffisom:sec:fast-algor-large}
...
13]{shoup93} and the proof of \cite[Theorem 2.1]{Shoup_1990}. Since $\phi$ commutes with traces, the 
...
$k(\zeta)$, for which we use Proposition~\ref{ffisom:prop:root_high_degree_extension}; this yields the 
main result of this section.
\begin{theorem}
	\label{ffisom:theorem:isom-root}
...
\begin{proof}
We can construct the embedding of Theorem~\ref{ffisom:theorem:isom-root} as
...
\begin{algorithm}
  \caption{Kummer-type algorithm for extension towers}
	\label{ffisom:algorithm:alpha-root}
	\begin{algorithmic}[1]
		...
		\STATE\label{ffisom:step:factor-cyclo} Factor the $v$-th cyclotomic polynomial over $\F_q$ to 
		build the extensions $k(\zeta) / \F_q(\zeta)$ and $K(\zeta)/\F_q(\zeta)$;
		\STATE\label{ffisom:step:non-v-res} Find a random non $v$-th power $\eta \in \F_q(\zeta)$;
		\STATE\label{ffisom:step:rep-root1} Compute $r$-th roots $\theta_1,\theta_2$ of $\eta$ in $k(\zeta),K(\zeta)$;
		\STATE\label{ffisom:step:trace} Compute $\alpha = \trace_{k(\zeta) / k}(\theta_1)$ and $\beta = 
		\trace_{K(\zeta) / 
		K}(\theta_2)$;
		\RETURN The field embedding defined by $\alpha\mapsto\beta$.
	\end{algorithmic}
...
Step~\ref{ffisom:step:factor-cyclo} is done using~\cite[Theorem~9]{shoup94},
which takes $O(\MM(v)\log(vq))$ operations in $\F_q$. We do
Step~\ref{ffisom:step:non-v-res} by taking random elements in $\F_q(\zeta)$
...
Section~\ref{ffisom:sec:preliminaries}.

Step~\ref{ffisom:step:rep-root1} is done using $d = O(\log(r) / \log(v))$ successive root extractions, 
...
in $k$ and $K$, reps., we see that computing the traces of Step \ref{ffisom:step:trace} boils down to 
computing the traces $\trace_{\F_q(\zeta) / \F_q}(\zeta^i)$ for $0 \le i < s$. The cost of this is 
dominated by those of the previous steps. Therefore, Algorithm~\ref{ffisom:algorithm:alpha-root} runs in 
...
\label{ffisom:sec:rains-algorithm}
...
idea in greater detail in Section~\ref{ffisom:sec:rains-elliptic}.
...
Note~\ref{ffisom:note:rains-non-prime}.
...
\item find a \emph{small} $\ell$ satisfying the conditions of
  Lemma~\ref{ffisom:th:gaussian} with $[\F_q(\mu_\ell):\F_q]=r$;
...
Lemma~\ref{ffisom:th:gaussian} imply that $\ell$ divides $\Phi_r(q)$, thus in
...
Algorithm~\ref{ffisom:algorithm:rains-cyclo}; we only give the procedure for
the field $k$, the procedure for the field $K$ being identical.

\begin{algorithm}
  \caption{Rains' cyclotomic algorithm}
  \label{ffisom:algorithm:rains-cyclo}
  \begin{algorithmic}[1]
    ...
    \REPEAT
    \STATE\label{ffisom:algorithm:rains-cyclo:power} Compute $\zeta\leftarrow \theta^{(\#k'-1)/\ell}$ for a random $\theta\in k'$
    \UNTIL $\zeta$ is a primitive $\ell$-th root of unity;
    \STATE\label{ffisom:algorithm:rains-cyclo:period} Compute $\eta(\zeta) \leftarrow \sum_{\sigma\in S}\zeta^\sigma$;
    \RETURN\label{ffisom:algorithm:rains-cyclo:trace} $\alpha \leftarrow \trace_{k'/k}\eta(\zeta) = \sum_{i=0}^{s-1}\eta(\zeta)^{q^{ri}}$.
  \end{algorithmic}
\end{algorithm}

\begin{proposition}
  Algorithm~\ref{ffisom:algorithm:rains-cyclo} is correct. On input
...
\begin{proof}
  ...
  Lemma~\ref{ffisom:th:gaussian} $\eta(\zeta)$ is a normal generator of $k'$,
  and by~\cite[Prop.~5.2.3.1]{mullen2013handbook} $\alpha$ is a
  normal generator of $k$. This proves correctness.

  According to Proposition~\ref{ffisom:prop:trace-like}, computing $\zeta$
  in Step~\ref{ffisom:algorithm:rains-cyclo:power}
  costs
  ...
  Steps~\ref{ffisom:algorithm:rains-cyclo:period}
  and~\ref{ffisom:algorithm:rains-cyclo:trace} can be performed at once by
  observing that
  ...
  using the techniques of Section~\ref{ffisom:sec:preliminaries}.
...
over the extension $k$ following case~$5$ from Section~\ref{ffisom:sec:fundamentalgo}.
...
questions will be analyzed in Section~\ref{ffisom:sec:selection}.

\begin{note}
  \label{ffisom:note:rains-non-prime}
...
\label{ffisom:sec:rains-elliptic}
...
a weak counterpart to Lemma~\ref{ffisom:th:gaussian}, is left as an open problem.

\subsection{Uniquely defined orbits from elliptic periods}
\label{ffisom:sec:ellperiods}
...
\begin{definition}
  \label{ffisom:definition:ellperiod}
...
\begin{lemma}
  \label{ffisom:lemma:ellperiods-order}
  With the same notation as in Definition~\ref{ffisom:definition:ellperiod},
  let
  \begin{equation*}
...
All known proofs of Lemma~\ref{ffisom:th:gaussian} rely on the fact that
...
$\F_q(x(P))$, that is a weak counterpart to Lemma~\ref{ffisom:th:gaussian}
...
Conjecture~\ref{ffisom:conj:ellperiods} is false.
...
Algorithm~\ref{ffisom:algorithm:rains-cyclo}, and follows immediately from
...
\item find a prime $\ell$, an elliptic curve $E$, and an eigenvalue
  ...
  of Definition~\ref{ffisom:definition:ellperiod}, and such that
  $\order_\ell(\lambda)=r$;
...
Section~\ref{ffisom:sec:selection}. Here we suppose that we are already given
...
\label{ffisom:algorithm:compell}
  \begin{algorithmic}[1]
...
\begin{proposition}
  Algorithm~\ref{ffisom:algorithm:compell} is correct. Assuming the
...
\begin{proof}
  ...
  Lemma~\ref{ffisom:lemma:ellperiods-order}. Success probability comes from
...
\item $(\Z/\ell\Z)^\times = \langle q\rangle \times S$ for some $S$,
\item $\langle q \rangle = rs$ for some integer $s$,
\item $\gcd(\euler(\ell),d)=1$ (see Note~\ref{ffisom:note:rains-non-prime}).
\end{enumerate}
...
if $\gcd(\ell,r)\ne1$, then the algorithms of Section~\ref{ffisom:sec:kummer} have much
...
\begin{remark}
  ...
  Section~\ref{ffisom:sec:fast-artin-schreier}, we thus ignore it.
\end{remark}

In the elliptic algorithm we look for an integer $\ell$ and a curve
$E/\F_q$ that satisfy the preconditions of
Algorithm~\ref{ffisom:algorithm:compell}, i.e., such that 
\begin{enumerate}
...
We summarize the elliptic case in Algorithm~\ref{ffisom:algorithm:selectell}.

\begin{algorithm}
  \caption{Parameter selection for Rains' elliptic algorithm}
  \label{ffisom:algorithm:selectell}
  \begin{algorithmic}[1]
    ...
    \FOR{$u=1$ to $\bar{u}$}
    \IF{\label{ffisom:alg:selectell:prime} $\gcd(u,r)=1$, and $\ell=ur+1$ is prime, and $q^r\ne1\bmod\ell$}
    \STATE\label{ffisom:alg:selectell:ellorder} Compute $\mathcal{T} \leftarrow \{\lambda + q/\lambda \bmod\ell \;|\; \order_\ell(\lambda)=r\}$;
    \FOR{each curve $E$ defined over $\F_q$}
    \STATE\label{ffisom:alg:selectell:pointcount} Compute $t = \#E - q + 1$;
...
Figure~\ref{ffisom:fig:primes-arith-prog} we plot for all prime powers
...
\begin{figure}
  ...
  \caption{Prime powers $r$ (abscissa) versus smallest integer $u$
    ...
    logarithmic scale.}
  \label{ffisom:fig:primes-arith-prog}
\end{figure}
...
Algorithm~\ref{ffisom:algorithm:selectell}. %
...
\label{ffisom:sec:experimental-results}
...
Section~\ref{ffisom:sec:fast-kummer} were implemented in C on top of the
Flint library, version 2.5.2~\cite{Hart2010}. %
...
2.40GHz. %
We report in Figure~\ref{ffisom:fig:bench} some statistics, exclusively on
...
\begin{figure}
  ...
  \begin{subfigure}{.48\textwidth}
    ...
    \label{ffisom:fig:bench:allombert-lowaux}
  \end{subfigure}
  \hfill
  \begin{subfigure}{.48\textwidth}
    \noindent
    ...
    \caption{Comparison of various implementations of Allombert's
      ...
      linear regressions.}
    \label{ffisom:fig:bench:allombert-anyaux}
  \end{subfigure}
  ...
  \begin{subfigure}{.48\textwidth}
    ...
    \caption{Comparison of Allombert's (Automorphism evaluation
      ...
      and maximum times.}
    \label{ffisom:fig:bench:all}
  \end{subfigure}

  \caption{Benchmarks for Rains' and Allombert's algorithms. $q$ is a
    prime between $100$ and $2^{20}$, $r$ is an odd prime power
    varying between $3$ and $2069$.  Plots~\subref{ffisom:fig:bench:rains}
    and~\subref{ffisom:fig:bench:all} are in doubly logarithmic scale.}
  \label{ffisom:fig:bench}
\end{figure}
...
\begin{figure}
  ...
  \label{ffisom:fig:bench:magma}
\end{figure}

We start by comparing our implementation of the three variants of
Allombert's algorithm presented in Section~\ref{ffisom:sec:algor-rely-polyn}
with the original one in PARI. %
In Figure~\ref{ffisom:fig:bench:allombert-lowaux} we plot running times
...
cost in $r$, in Figure~\ref{ffisom:fig:bench:allombert-anyaux} we take
...
described in Section~\ref{ffisom:sec:algor-rely-line}; this is the curve
...
Section~\ref{ffisom:sec:algor-rely-polyn}, one can't fail to notice that the
...
Figure~\ref{ffisom:fig:bench:magma}); hence we will compare these algorithms
only based on our timings. %
In Figure~\ref{ffisom:fig:bench:rains} we group runs of the cyclotomic
...
variant (see Appendix~\ref{ffisom:app:rains-vars}). %
...
Finally, in Figure~\ref{ffisom:fig:bench:all} we compare Rains' algorithms
against Allombert's. %
...
time. %
We also take from Figure~\ref{ffisom:fig:bench:rains} the graphs for the
...
Section~\ref{ffisom:sec:experimental-results}. %
...
This is summarized in Algorithm~\ref{ffisom:algorithm:rains-conic}.

\begin{algorithm}
    \caption{Rains' conic algorithm}
  \label{ffisom:algorithm:rains-conic}
  \begin{algorithmic}[1]
    ...
    \UNTIL\label{ffisom:algorithm:rains-conic:sqtest} $\theta^2-4$ is a quadratic non-residue;
    \STATE\label{ffisom:algorithm:rains-conic:power} Compute $\zeta=[(\#k'+1)/\ell]\theta$,
    \UNTIL $\zeta\ne2$;
    \STATE\label{ffisom:algorithm:rains-conic:period} Compute $\eta(\zeta) \leftarrow \sum_{\sigma\in S}[\sigma]\zeta$;
    \RETURN\label{ffisom:algorithm:rains-conic:trace} $\alpha \leftarrow \trace_{k'/k}\eta(\zeta) = \sum_{i=0}^{s-1}[q^{ri}]\eta(\zeta)$.
  \end{algorithmic}
\end{algorithm}

\begin{proposition}
  Algorithm~\ref{ffisom:algorithm:rains-conic} is correct: on input
  $q,r,\ell,s$ it returns an element in the same Galois orbit as
  Algorithm~\ref{ffisom:algorithm:rains-cyclo} on input $q,r,\ell,2s$. %
...
\begin{proof}
  ...
  Step~\ref{ffisom:algorithm:rains-conic:power} is the trace over $k'$ of a
  primitive $\ell$-th root of unity. %
  We conclude by comparing this algorithm with
  Algorithm~\ref{ffisom:algorithm:rains-cyclo}.

  The non-residuosity test in Step~\ref{ffisom:algorithm:rains-conic:sqtest}
  ...
  Hence, Step~\ref{ffisom:algorithm:rains-conic:power} costs
  $O(sr\MM(sr)\log(q))$ operations in $\F_q$, while
  Steps~\ref{ffisom:algorithm:rains-conic:period}
  and~\ref{ffisom:algorithm:rains-conic:trace} together cost
  $O((\MM(sr)(\ell/r)\log(\ell))$.
\end{proof}

Although this variant does not exploit the asymptotic improvement
offered by Proposition~\ref{ffisom:prop:trace-like}, the fact that its
auxiliary degree $s$ is half the one of the original algorithm usually
gives an interesting practical improvement. %
Step~\ref{ffisom:algorithm:rains-conic:power} can be modified so as to avoid
the premature projection on the $x$-axis, so that the algorithms of
Proposition~\ref{ffisom:prop:trace-like} apply. %
